...
\end{definition}
	...
	\begin{equation}
	    \label{mincut}
	    \textsc{Cut}(P_1, P_2, ..., P_k) := \frac{1}{2} \sum_{i=1}^k \partial(P_i)
	\end{equation}
...
\begin{align*}
	\textsc{RatioCut}(P_1, P_2, ..., P_k) &:= \frac{1}{2} \sum_{i=1}^k \frac{\partial\left(P_i\right)}{\left| P_i \right|} \\
	&= \sum_{i=1}^k \frac{\textsc{Cut}(P_i, \overline{P_i})}{\left| P_i \right|}
\end{align*}

\begin{align*}
	\textsc{NormCut}(P_1, P_2, ..., P_k) & := \frac{1}{2} \sum_{i=1}^k \frac{\partial\left(P_i\right)}{\textsc{Vol}(P_i)}\\
